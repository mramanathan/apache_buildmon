...
def buildsInfo(jobInfo, dbConn, cursor):
    """
    param : mysqlconnection plugs, and global projectInfo
    Retrieve build ids and URLs for each project and save
    the data in a dict and also in mysql dB.
    
    return : none
    """
    
    for prjName in projectInfo.iterkeys():
        prjInfo = jobInfo.get_job(prjName)
        prjBuilds = prjInfo.get_build_dict()
        for buildId, buildUrl in prjBuilds.iteritems():
            cursor.execute("""INSERT INTO Builds(name, jobid, joburl) VALUES(%s, %s, %s)""",
                        (prjName, buildId, buildUrl))
